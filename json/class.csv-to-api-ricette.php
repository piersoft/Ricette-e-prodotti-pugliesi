  ...
  function parse() {
    ...
    if ( !$this->data ) {
      ...
      if (ini_get('allow_url_fopen') == true) {
        $ricezionedata=file_get_contents( $this->source );
        $this->data=strtolower($ricezionedata);
        $this->data=str_replace('tipicita','Tipicita',$this->data);  //fix per le foto
  ...
  function list_filter( $list, $args = array(), $operator = 'AND' ) {
    ...
    foreach ( $list as $key => $obj ) {
      ...
      foreach ( $args as $m_key => $m_value ) {
        ...
        //if ( array_key_exists( $m_key, $to_match ) && $m_value == $to_match[ $m_key ] ) // originale
        if ( array_key_exists( $m_key, $to_match ) && stripos(strtolower($to_match[ $m_key ]),$m_value) !== false){
          //  $filtered['id']=  $matched;
            $matched++;
        }
